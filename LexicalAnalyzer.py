...
# to identify the reserved keywords
...
class LEX_STATE:
    """
    Represents the lexical state.
    """
    def __init__(self):
        self.ln_num = 0
        self.cur_character = None
        self.col_num = None
...
class Tokenizer:
    ...
    def __init__(self, text):
        ...
        self.state.cur_character = self.text[self.position]
        self.state.ln_num = 1
        self.state.col_num = 1

    def error(self):
        """
        Raises an exception for parsing errors.
        """
        raise Exception('Error parsing input')

    def advance(self):
        """
        Advances the position in the text.
        """
        self.position += 1
        if self.position > len(self.text) - 1:
            self.state.cur_character = None  # Indicates end of input
        else:
            self.state.cur_character = self.text[self.position]
            self.state.col_num += 1
    ...
    def skip_whitespace(self):
        ...
        """
        while self.state.cur_character is not None and self.state.cur_character.isspace():
            if self.state.cur_character == '\n':
                self.state.ln_num += 1
    ...
    def integer(self):
        ...
        """
        result = ''
        while self.state.cur_character is not None :
            if self.state.cur_character.isdigit():
                result += self.state.cur_character
                self.advance()
            elif self.state.cur_character.isalpha():
                self.error()
            else: break
    ...
    def identifier(self):
        ...
        """
        result = ''
        while self.state.cur_character is not None and (
                self.state.cur_character.isalpha() or self.state.cur_character.isdigit() or self.state.cur_character == '_'):
            result += self.state.cur_character
            self.advance()
        return result
    ...
    def comment(self):
        ...
        """
        result = ''
        while self.state.cur_character is not None and self.state.cur_character != '\n':
            result += self.state.cur_character
            self.advance()
        return result
    ...
    def get_next_token(self):
        ...
        while self.state.cur_character is not None:
            ...
            # skip whitespaces
            if self.state.cur_character.isspace():
                self.skip_whitespace()
                continue

            # tokenize digits
            elif self.state.cur_character.isdigit():
            ...
            elif self.state.cur_character in PUNCTION:
                token = Token(TokenType.PUNCTION, self.state.cur_character)
                self.advance()
                return token
...
class Screener:
    """
    Merges tokens based on certain rules.
    """
