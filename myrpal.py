...
from LexicalAnalyzer import Screener
...
class ASTParsser:
    """
    A class that represents an Abstract Syntax Tree (AST) parser.

    Attributes:
    - tokens: A list of tokens to be parsed.
    - current_token: The current token being processed.
    - index: The index of the current token in the tokens list.
    """
    ...
    def __int__(self, tokens1):
        ...
        Parameters:
        - tokens1: A list of tokens to be parsed.
        """
        self.tokens = tokens1
    ...
    def read(self):
        """
        Read the next token and update the current_token attribute.
        """
        # Code omitted for brevity

    def buildTree(self, token, ariness):
        """
        Build an Abstract Syntax Tree (AST) node with the given token and arity.

        Parameters:
        - token: The token value of the node.
        - ariness: The arity of the node.
        """
        # Code omitted for brevity
...
class ASTParsser:
    ...
    def read(self):
        ...
        if self.current_token.type in [LexicalAnalyzer.TokenType.ID, LexicalAnalyzer.TokenType.INT,
                                       LexicalAnalyzer.TokenType.STRING] :

            terminalNode = AbstractSyntaxTreeNode( str(self.current_token.type))
        ...
        if self.current_token.value in  ['true', 'false', 'nil', 'dummy']:
            # stack.append(AbstractSyntaxTreeNode(self.current_token.value))
            terminalNode = AbstractSyntaxTreeNode(str(self.current_token.type))
            terminalNode.value = self.current_token.value
            stack.append(terminalNode)
    ...
    def buildTree(self, token, ariness):
        ...
        node = AbstractSyntaxTreeNode(token)
        ...
        while ariness > 0:
            ...
            children = stack[-1]
            stack.pop()
            # Assuming pop() is a function that returns an AbstractSyntaxTreeNode
            if node.children is not None:
                children.sib = node.children
                node.children.previous = children
                # node.previous = children
            node.children = children

            node.sourceLineNumber = children.sourceLineNumber
            ariness -= 1
        # node.print_tree()
    ...
    def procE(self):
        ...
        match self.current_token.value:
            ...
            case 'fn':
                ...
                while self.current_token.type == LexicalAnalyzer.TokenType.ID or self.current_token.value == '(':
                    self.procVb()
                    n += 1
    ...
    def procTc(self):
        ...
        self.procB()
        # print('Tc->B')
        if self.current_token.type == LexicalAnalyzer.TokenType.TERNARY_OPERATOR:
            self.read()
            self.procTc()
    ...
    def procR(self):
        ...
        while (self.current_token.type in [LexicalAnalyzer.TokenType.ID, LexicalAnalyzer.TokenType.INT,
                                           LexicalAnalyzer.TokenType.STRING] or self.current_token.value in ['true', 'false',
                                                                                                        'nil', 'dummy',
                                                                                                        "("]):
            if self.index >= len(self.tokens):
                break
    ...
    def procDb(self):
        ...
        elif self.current_token.type == LexicalAnalyzer.TokenType.ID:
            self.read()

            if self.current_token.type == LexicalAnalyzer.TokenType.COMMA:
                # Db -> Vl '=' E => '='
                self.read()
                self.procVb()
    ...
    def procVb(self):
        # print('procVb')
        if self.current_token.type == LexicalAnalyzer.TokenType.ID:
            self.read()
            # print('Vb->id')
            # self.buildTree("id", 1)
    ...
    def procVL(self):
        ...
        if self.current_token.type != LexicalAnalyzer.TokenType.ID:
            ...
            # print('498')
            if trees_to_pop > 0:
                self.buildTree(',', trees_to_pop +1)  # +1 for the children identifier
...
if __name__ == "__main__":
    ...
    # tokenize input
    tokenizer = LexicalAnalyzer.Tokenizer(program)
    token = tokenizer.get_next_token()
    while token.type != LexicalAnalyzer.TokenType.EOF:
        tokens.append(token)
        token = tokenizer.get_next_token()
    ...
    if ast_flag == 0:
        ASTStandarizer = AbstractSyntaxTreeNode("ASTStandarizer")
        root = ASTStandarizer.standard(root)
